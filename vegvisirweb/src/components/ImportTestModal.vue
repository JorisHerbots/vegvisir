            ...
            <div id="test" class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 space-y-1">
                    ...
                    <div id="listOfImplementationsToAdd" class="h-full">
                        <ul>
                            <li v-for="(item, index) in filteredList" @click="ClickedTest(item)" style="list-style: none" class="text-3xl">
                                {{item}}               
                            </li> 
                        </ul>
                    </div>
...
import { useTestsStore } from '@/stores/UseTestsStore';
    ...
    export default {
      name: "ImportTestModal",
      setup() {
        const testsStore = useTestsStore();
        testsStore.getAllImportableTests();
      ...
      methods: {
        ...
        ClickedTest(name : string) {
            this.testsStore.importTestReproducable(name);
            this.$router.push({ path: '/progress' })
        },
      },
      computed: {
        filteredList() {
          let list = Object.values(this.testsStore.importable_test_names)
          return list.filter(implementation => {
            return implementation.toLowerCase().includes(this.search.toLowerCase())
          })
        }
      },
